...
// Get available payments (VENTAS ECOMMERCE movements not attached to any ecommerce pedido)
  ...
  try {
    const result = await pool.query(`
      ...
      WHERE le.area = 'VENTAS'
        AND le.subarea = 'VENTAS ECOMMERCE'
        AND le.entry_type = 'income'
        AND le.id NOT IN (
          SELECT ledger_entry_id FROM ecommerce_pedido_payments
        )
    ...
    console.error('Error getting available ecommerce payments:', error);
...
export const getPedidoPayments = async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(`
      SELECT
        epp.id as attachment_id,
        epp.attached_at,
        epp.notes as attachment_notes,
      ...
      LEFT JOIN users u ON epp.attached_by = u.id
      LEFT JOIN users creator ON le.user_id = creator.id
      WHERE epp.pedido_id = $1
      ORDER BY le.transaction_date DESC, epp.attached_at DESC
    `, [id]);
    ...
    console.error('Error getting ecommerce pedido payments:', error);
...
export const attachPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Verify the ledger entry exists and is VENTAS ECOMMERCE income
    ...
    if (ledgerEntry.area !== 'VENTAS' || ledgerEntry.subarea !== 'VENTAS ECOMMERCE') {
      throw new Error('Ledger entry is not from VENTAS ECOMMERCE area');
    }
    ...
    const existingAttachment = await client.query(`
      SELECT id FROM ecommerce_pedido_payments WHERE ledger_entry_id = $1
    `, [ledger_entry_id]);
    ...
    const pedidoCheck = await client.query(`
      SELECT id, total FROM ecommerce_pedidos WHERE id = $1
    `, [pedido_id]);
    ...
    const result = await client.query(`
      INSERT INTO ecommerce_pedido_payments (pedido_id, ledger_entry_id, attached_by, notes)
      VALUES ($1, $2, $3, $4)
      RETURNING *
    ...
    console.error('Error attaching ecommerce payment:', error);
...
export const detachPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const attachmentCheck = await client.query(`
      SELECT id, pedido_id FROM ecommerce_pedido_payments WHERE id = $1
    `, [id]);
    ...
    await client.query(`
      DELETE FROM ecommerce_pedido_payments WHERE id = $1
    `, [id]);
    ...
    await client.query('ROLLBACK');
    console.error('Error detaching ecommerce payment:', error);
  ...
  try {
    const result = await pool.query(`
      SELECT
        ...
        p.amount_paid,
        p.payment_status,
        (p.total - COALESCE(p.amount_paid, 0)) as amount_remaining,
        (SELECT COUNT(*) FROM ecommerce_pedido_payments WHERE pedido_id = p.id) as payment_count
    ...
    console.error('Error getting ecommerce payment summary:', error);
